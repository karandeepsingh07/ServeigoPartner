...
public class AvailableJobsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter=new AdapterJobAvailableList((ArrayList<ClassBooking>) listItems,this,state,city,category,subCategory,vendorID);
    ...
    private void getResult(final String state,final String city, final String category, final String subCategory, final String vendorID){
        firebaseFirestore.collection("Vendor").document(vendorID).collection("Job").whereEqualTo("status","Open").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if(task.isSuccessful()) {
                    ...
                    for (final DocumentSnapshot snapshot : task.getResult()) {
                        ...
                        firebaseFirestore.collection("Job").document("Open").collection("UID").document(snapshot.getId()).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                            @Override
...
//                                Log.d("xyz", classBooking.getUserName());
                                ...
                                Log.d("jobid",classBooking.getJobId());
